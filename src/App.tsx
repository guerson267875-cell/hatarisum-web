...
export default function App() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          ...
          <div className="col-span-12 md:col-span-6 space-y-4">
            <Wordmark className="text-[clamp(1.35rem,1rem+1.6vw,2rem)]" />
      ...
      {/* BANNER DE COOKIES — barra delgada full-width */}
      <AnimatePresence>
        {showCookieBanner && (
          <motion.div
            id="cookie-bar"
            ...
            className="fixed bottom-0 left-0 right-0 z-50 bg-[#070E1A]/90 border-t border-white/[0.08] backdrop-blur-xl px-4 md:px-8 py-2.5 flex items-center justify-between gap-3 select-none"
          >
            <p className="text-[11px] md:text-xs text-zinc-300 font-sans font-light leading-snug">
              Usamos cookies para que tu visita funcione mejor.{' '}
              <a
                href="#privacy"
                onMouseEnter={() => handleCursorHover(true)}
                onMouseLeave={() => handleCursorHover(false)}
                className="underline hover:text-white cursor-none text-zinc-400"
              >
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                onMouseEnter={() => handleCursorHover(true)}
                onMouseLeave={() => handleCursorHover(false)}
                className="cursor-none px-4 py-1.5 bg-[#B8442A] text-white rounded-full text-[11px] font-sans font-medium hover:bg-[#D4593A] transition-colors duration-200"
              >
                Aceptar
              </button>
